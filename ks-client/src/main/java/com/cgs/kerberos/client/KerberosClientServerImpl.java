...
public class KerberosClientServerImpl implements KerberosClientServer {
    ...
    public ThirdRequest getThirdRequest(SecondResponseWrapper secondResponseWrapper) throws KerberosException {
        ...
        byte[] encryptedThirdRequestInformation = SecurityUtil
                .encryptAes(decryptedThirdRequestInformation, serviceSessionKey);
    ...
    public byte[] checkServiceTicket(byte[] thirdRequestByte) throws KerberosException {
        ...
        ThirdRequestInformation thirdRequestInformation = (ThirdRequestInformation) serializer
                .byte2Object(decryptedThirdRequestInfo);
        ...
        if (clientName1 == null || clientName2 == null || !clientName1.equals(clientName2)) {
            throw new InvalidStException(
                    "Invalid Service Ticket. Request client name is different from service ticket's client name. Client name is "
                            + clientName2 + " Service ticket's client name is " + clientName1);
        }
        ...
        if (expiredTime > 0) {
            if (TimeComparator.isInDifference(thirdRequestInformation.getTimeStamp(), expiredTime)) {
                throw new InvalidStException(
                        "Invalid Service Ticket.The request time is " + thirdRequestInformation.getTimeStamp()
                                + ". The sever's system time is "
    ...
    @Override
    public boolean checkServiceResponse(byte[] thirdResponse, SecondResponseWrapper secondResponseWrapper)
